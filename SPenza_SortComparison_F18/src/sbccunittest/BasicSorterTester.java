...
/**
 * Unit test code to grade the sort comparison assignment. 11/6/2018
 * 
 * @author spstrenn
 */
public class BasicSorterTester {
	...
	public void testPartition() {
		var words = createRandomStrings(1024, 5);
	...
	public void testHeapify() {
		var words = createRandomStrings(100000, 5); // 1e6 -> 0.66 sec, 4e6 -> 3.53
													// sec
	...
	void standardSortTest(int sortId, int wordLength, int minN, int maxN, int stepN, int numSamplesPerTest,
		...
		println(dataOrder == SORTED_DATA ? " sorted data" : " random data");
	...
	private void verifyIsHeap(String[] words, int i) {
		...
		if (words[i].compareTo(words[2 * i + 1]) < 0)
			fail("heapify() failed.  The parent (" + words[i] + ") is less than its left child (" + words[2 * i + 1]
					+ ").");
	...
	public String[] createRandomStrings(int n, int wordLength) {
		...
		for (int ndx = 0; ndx < n; ndx++)
			words[ndx] = RandomStringUtils.randomAscii(wordLength);
		return words;
	}
